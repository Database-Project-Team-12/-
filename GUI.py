...
#################### Functions for search button ###############################################################################
            ...
            for value in values:
                if value.isdigit():
                    ...
                    try:
                        ...
                        if "drop table" in value.lower():
                            messagebox.showerror("Error", "The query contains 'DROP TABLE', which is not allowed.")
                            return
                        try:
    ...
    def custom_query(self, directory, user_input, current_tab): 
        ...
        while i < len(user_input_words):
            ...
            if word in sql_schema_dict_lemmatized.values():  # Check if the word exists in the dictionary values before lemmatizing it
...
################# Generic Purpose Functions for enter/back buttons #############################################################         
    ...
    def check_type(self, type, value):
        if type == 'integer':
            ...
            try:
                datetime.strptime(value, '%Y-%m-%d')  # Check if the date is in 'year-month-date' format
                return True
            except ValueError:
                return False
