...
class SQL:
    ...
    def find_default_values(self, sql_command):
        ...
        # Regular expression to match column name, NOT NULL constraint, and optional default value
        pattern = r'"(\w+)"\s+\w+(?:\(\d+\))?(?:\s+(NOT NULL))?(?:\s+DEFAULT\s+(\w+|NULL|\'\w+\'))?'
        matches = re.findall(pattern, sql_command)

        # Create an OrderedDict where the key is the column name and the value is the default value or None if no default value
        # If the column has NOT NULL constraint, append an * to the column name
        default_values = OrderedDict((column + ('*' if not_null else ''), default if default else 'None') for column, not_null, default in matches)
...
class SQL_queries():
    ...
    def create_sql_queries_with_group_by(self):
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            f.write("SELECT ANIMAL.Animal_ID, ANIMAL.Species, ANIMAL.Name, COUNT(*) AS Lunches_Per_Animal FROM ANIMAL LEFT OUTER JOIN EATING ON ANIMAL.Animal_ID = EATING.Animal_ID GROUP BY EATING.Animal_ID;\n\n")
            ...
            f.write("SELECT ANIMAL.Species, ANIMAL.Name, COUNT(*) AS Cases_Per_Animal FROM ANIMAL JOIN VET ON ANIMAL.Animal_ID = VET.Animal_ID GROUP BY VET.Animal_ID;\n\n")
