...
# Try to create a lemmatizer
try:
    lemmatizer = WordNetLemmatizer()
except LookupError:
    # If the WordNet resource is not found, download it and then create a lemmatizer
    nltk.download('wordnet')
    lemmatizer = WordNetLemmatizer()
...
#################### Functions for search button ###############################################################################
    ...
    def automate_tabview_creation(self, window, table_name, text=None, iter=None):
        ...
        """
        widgets = []
        entries,labels = self.Automate_EntryBox_creation(working_window=window, table_name=table_name, iter=iter)
        ...
        # Create a switch
        self.switch = ctk.CTkSwitch(window, text="Build-in Queries", command=self.switch_event_on_tabview, 
        ...
        if values is None:
            ...
            for value in values:
                if value.isdigit():
                    ...
                    try:
                        value = value.lower()
                        value = self.search_files("Queries", value, table_name)
    ...
    # Search for a match in sql files
    def search_files(self, directory, user_input, current_tab): 
        """
        This function searches for SQL files in a given directory and returns the sentence that best matches the user input.
        The user input is first converted to SQL keywords using a dictionary read from a text file.
        Each SQL file is read and split into sentences, and each sentence is compared to the user input.
        If a perfect match is found, the matching sentence is returned immediately.
        Otherwise, the function keeps track of the sentence that has the highest similarity ratio to the user input, and returns this sentence at the end.

        Args:
            directory (str): The directory in which to search for SQL files.
            user_input (str): The user input to match against the sentences in the SQL files.

        Returns:
            str: The sentence from the SQL files that best matches the user input.
        """
        ...
        user_input_words = re.split('(\W+)', user_input.lower())

        # Replace 'id' with 'Animal_ID' in the user input
        user_input_words = [current_tab.lower().capitalize() + '_ID' if word == 'id' else word for word in user_input_words]

        # For each word or pair of words in the user input, try to find a match in the dictionaries
        i = 0
        while i < len(user_input_words):
            word = lemmatizer.lemmatize(user_input_words[i])
            pair = ' '.join(lemmatizer.lemmatize(word) for word in user_input_words[i:i+2]) if i < len(user_input_words) - 1 else None
            if word in sql_schema_dict_lemmatized:
                user_input_words[i] = sql_schema_dict_lemmatized[word]
                i += 1
            elif pair and pair in sql_schema_dict_lemmatized:
                user_input_words[i] = sql_schema_dict_lemmatized[pair]
                del user_input_words[i+1]
            elif word in human_to_sql:
                user_input_words[i] = human_to_sql[word]
                i += 1
            else:
                i += 1
        ...
        for filename in sql_files:
            with open(filename, 'r') as file:
                ...
                for sentence in sentences:
                    ratio = difflib.SequenceMatcher(None, user_input, sentence).ratio()
                    if ratio > highest_ratio:
                        highest_ratio = ratio
                        closest_match = filename
                        closest_sentence = sentence

        # Check if the user input and the closest_sentence match
        if closest_sentence != user_input:
            return user_input
...
################# Functions for New Entry button ###############################################################################
    ...
    def refresh(self, event=None):
        ...
        self.entries,widgets = self.Automate_EntryBox_creation(working_window=self.entry_window,
                                                               table_name=selection, entry_text="Default value:" ,text=text)
        height = len(widgets)*self.height/12
...
################# Functions for the right/table frame ##########################################################################
    ...
    def Automate_EntryBox_creation(self, working_window, table_name, entry_text=None, text=None, iter=None):
        """
        ...
        self.enter_button.place(relx=0.7, rely=0.95, anchor='center')
        
        # Add Back Button
        self.backwards_button = ctk.CTkButton(working_window, font=("Arial", 12), text="Go Back")
        self.backwards_button.place(relx = 0.3, rely=0.95, anchor='center')
    ...
    def edit_button_event(self):
        ...
        entries,widgets = self.Automate_EntryBox_creation(working_window=self.edit_window, 
                                                          table_name=self.last_pressed_button, entry_text="Enter here:" ,text=text)
...
################# Generic Purpose Functions that deal with SQL.py ##############################################################    
    ...
    def sql_command(self,selection): 
        """
        Finds the SQL command for creating the specified table and extracts the default values for the table's columns. The SQL 
        commands are split by semicolon.The default values are then extracted from this command and returned as an OrderedDict.

        Args:
            selection (str): The name of the table for which to find the CREATE TABLE command and extract the default values.

        Returns:
            OrderedDict: The default values for the table's columns, with the column names as keys and the default 
            values as values.
        """
        ...
        default_values = self.sql.find_default_values(sql_command)
        # Convert default_values to an OrderedDict
...
################# Generic Purpose Functions for enter/back buttons #############################################################         
    ...
    def get_entry_value(self, window, entry_widgets):
        """
        Retrieves the values from the provided entry widgets. If a value starts with "Type here to", it is skipped. If a value 
        contains "Default value:", the previous or default value associated with the label text of the entry is retrieved and 
        used instead.
        ...
        Returns:
            list: The values from the entry widgets, with entries containing ("Default value:") values replaced with the 
            appropriate previous or default values based on the needs of the window.
        """
        values = []
        for entry in entry_widgets:
            entry_value = entry.get()
            if entry_value.startswith("Type here "):
                continue
            label_widget = self.label_entry_dict[entry]
            label_text = label_widget.cget("text")
            if not label_text.endswith("*"):  # Check if label_text does not end with *
                if window == self.edit_window:
                    value = self.get_previous_values().get(label_text)
                else:  # window == self.entry_window
                    table_name = self.selected_value
                    value = self.sql_command(table_name).get(label_text)
                values.append(value)
            elif 'Default value:' not in entry_value:
    ...
    def get_previous_values(self):
        """
        Retrieves the previous values from the database for the table associated with the last pressed button. The values are 
        stored in a dictionary, which is also stored as an attribute of the object.

        Returns:
            dict: The previous values from the database, with the column names as keys and the values as values.
        """
        table_name = self.last_pressed_button
